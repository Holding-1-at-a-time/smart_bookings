...
import { handleConvexError } from "../../utils/errorHnadler";
...
describe("handleConvexError() handleConvexError method", () => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle null or undefined input gracefully", () => {
      ...
      expect(() => handleConvexError(null as any)).toThrow(MockAppError);
      expect(() => handleConvexError(null as any)).toThrow(
        expect.objectContaining({
          statusCode: 500,
          message: "An unexpected error occurred",
        }),
      ...
      expect(() => handleConvexError(undefined as any)).toThrow(MockAppError);
      expect(() => handleConvexError(undefined as any)).toThrow(
        expect.objectContaining({
          statusCode: 500,
          message: "An unexpected error occurred",
        }),
